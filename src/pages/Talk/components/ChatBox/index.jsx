...
const ChatBox = ({
  ...
  useEffect(() => {
    ...
    socket.emit('readMessage', {
      sendUsername: chatPersonInfo.username,
  ...
  return (
    <Layout className={style.chatBox}>
      {chatPersonInfo.username ? (
        <Layout>
          ...
          <Layout.Content style={{ overflowY: 'auto', overflowX: 'hidden' }}>
            {(messageList || []).map((messageItem, index) => {
              ...
              if (
                index !== 0 &&
                dayjs(messageItem.sendTime).isAfter(lastMessageTime) &&
                dayjs(messageItem.sendTime).isBefore(
                  lastMessageTime.add(3, 'm')
                )
              ) {
        ...
        <div className={style.center}>点击左侧头像选择聊天对象</div>
      )}
    </Layout>
  )
}
